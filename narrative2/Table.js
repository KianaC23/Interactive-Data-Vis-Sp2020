class Table {
  ...
  constructor(state, setGlobalState) {
    ...
    const slimmedData = state.data.map(d => ({
      ...
      "Total Directors": d['Total Directors']
    })).sort((a, b) => d3.descending(a['Total Directors'], b['Total Directors']))
    ...
    const logScale = d3
      ...
      .domain(d3.extent(slimmedData, d => d['Total Directors']))
    ...
    const columns = ["Distributors", "Total Directors"];
    ...
    this.tableRows = table
      ...
      .data(slimmedData)
      .join("tr")
      .style("background-color", d => this.colorScale(d['Total']))
      .style("color", "#eee");
    ...
    this.tableRows.on("click", d => {
      setGlobalState({ selectedState: d.Distributor });
    });
  }
  ...
  draw(state, setGlobalState) {
    ...
    this.tableRows.style("background-color", d =>
      state.selectedState === d.Distributor ? "grey" : this.colorScale(d['Total Directors'])
    );
  }
}
