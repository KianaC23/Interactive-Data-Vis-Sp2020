export function linechart(){
    ...
    * LOAD DATA
    * */
    d3.csv("../data/percentsByYear.csv", d => ({
    ...
    * */
    ...
    yScale = d3
    .scaleLinear()
    .domain([0, d3.max(state.data, d => d.percent)])
    ...
    .selectAll("option")
    .data([
        ...Array.from(new Set(state.data.map(d => d.demo))),
        default_selection,
    ])
    .join("option")
    .attr("value", d => d)
    ...
    * */
    ...
    filteredData = state.data.filter(d => d.demo === state.selected);
    }

    // update the scale domain (now that our data has changed)
    yScale.domain([0, d3.max(filteredData, d => d.percent)]);
    ...
    .call(
        ...
        selection
            ...
            .attr("cy", d => yScale(d.percent)) // started from the bottom, now we're here
    );
