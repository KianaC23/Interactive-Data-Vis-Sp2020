 ...
 * */
function init() {
  const container = d3.select("#treemap2").style("position", "relative");
  ...
  const uniqueDistributors = [...new Set(state.data.map(d => d.distributor))];
  const colorScale = d3
    .scaleOrdinal()
    .domain(uniqueDistributors)
    .range(d3.schemeSet3);
  ...
  leaf
    ...
    .attr("fill", d => colorScale(d.data[1].movies[0].distributor)) // take the genre from the first one in the group
