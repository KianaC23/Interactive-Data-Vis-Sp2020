class Count {
  ...
  draw(state, setGlobalState) {
    ...
    const filteredData = state.data.find(d => state.selectedState === d.Distributor);
    const metrics = ["Male Directors", "Female Directors", "UR Directors"];
    ...
    const metric = this.container
      ...
      .data(metricData, d => d.Distributor)
      .join(
        enter => 
          enter
            .append("div")
    ...
    metric.select("div.number")
      ...
      .style("color", d => d.metric === state.selectedMetric ? "red" : "#ccc")
