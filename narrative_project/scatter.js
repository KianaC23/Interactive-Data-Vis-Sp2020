 ...
 * LOAD DATA
 * */
d3.csv("../data/percentsByYear.csv", d3.autoType).then(raw_data => {
...
function init() {
  ...
  yScale = d3
    .scaleLinear()
    .domain(d3.extent(state.data, d => d.Percentage))
  ...
  selectElement
    .selectAll("option")
    .data(["All", "female", "male", "poc", "white"]) // unique data values-- (hint: to do this programmatically take a look `Sets`)
    .join("option")
    .attr("value", d => d)
    .text(d => d);
...
function draw() {
  ...
  if (state.selectedParty !== "All") {
    filteredData = state.data.filter(d => d.Entity === state.selectedParty);
  }
    ...
    .join(
      enter =>
        ...
        enter
          ...
          .attr("opacity", 0.5)
          .attr("fill", d => {
            if (d.Entity === "female") return "blue";
            else if (d.Entity === "male") return "red";
            else if (d.Entity === "poc") return "yellow";
            else return "white";
          })
          .attr("r", radius)
